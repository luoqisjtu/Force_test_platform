...
namespace ULAO04
{
	internal class frmStatusDisplay : System.Windows.Forms.Form
	{
		...
		private void InitUL()
		{
			...
			MemHandle = MccDaq.MccService.ScaledWinBufAllocEx(Count); // set aside memory to hold data
			if (MemHandle == IntPtr.Zero) 
				Application.Exit();
			...
			ULStat = MccDaq.MccService.ScaledWinArrayToBuf(DAData, MemHandle, FirstPoint, Count);
