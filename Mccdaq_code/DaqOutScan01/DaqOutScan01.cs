...
namespace DaqOutScan01
{
	internal class frmStatusDisplay : System.Windows.Forms.Form
	{
		...
		private void tmrCheckStatus_Tick(System.Object eventSender, System.EventArgs eventArgs)
		{
			...
			//   FunctionType: Daq output operation (MccDaq.FunctionType.DaqoFunction)
		...
		private void InitUL()
		{
			...
			MemHandle = MccDaq.MccService.WinBufAllocEx(Count); // set aside memory to hold data
			if (MemHandle == IntPtr.Zero) 
				Application.Exit();
			...
			ULStat = MccDaq.MccService.WinArrayToBuf(DAData, MemHandle, FirstPoint, Count);
